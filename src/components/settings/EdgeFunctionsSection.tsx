...
const EDGE_FUNCTIONS: EdgeFunctionDoc[] = [
  ...
  { name: 'clickup-import', description: 'Importa tasks do ClickUp como projetos. Suporta list_id (lista inteira) ou task_id (task individual). Mapeamento automático de status ClickUp → fase/status do sistema via auto_map_status.', category: 'Importação', method: 'POST', auth: 'jwt', samplePayload: '{"task_id":"abc123","mode":"import","project_type":"venda","auto_map_status":true}', notes: 'Requer CLICKUP_API_TOKEN. Parâmetros: list_id OU task_id (obrigatório um dos dois), mode (preview/import), auto_map_status (true mapeia status ClickUp → fase/status automaticamente), project_type, phase_name, status (fallback se auto_map não encontrar correspondência).' },
