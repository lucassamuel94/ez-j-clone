...
-- Drop old 7-param overload to avoid ambiguity
DROP FUNCTION IF EXISTS public.search_opportunities_kanban(text, uuid, text, text, integer, integer, text);
...
CREATE OR REPLACE FUNCTION public.search_opportunities_kanban(
  ...
  p_meeting_from text DEFAULT NULL,
  p_meeting_to text DEFAULT NULL,
  p_won_from text DEFAULT NULL,
  p_won_to text DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE o.returned_to_sdr = false
      ...
      END
      AND CASE
        WHEN p_meeting_from IS NOT NULL THEN o.meeting_datetime >= p_meeting_from::timestamptz
        ELSE true
      END
      AND CASE
        WHEN p_meeting_to IS NOT NULL THEN o.meeting_datetime <= p_meeting_to::timestamptz
        ELSE true
      END
      AND CASE
        WHEN p_won_from IS NOT NULL THEN o.won_at >= p_won_from::timestamptz
        ELSE true
      END
      AND CASE
        WHEN p_won_to IS NOT NULL THEN o.won_at <= p_won_to::timestamptz
        ELSE true
      END
      AND CASE
