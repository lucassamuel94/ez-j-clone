...
BEGIN
  ...
  SELECT DISTINCT ON (COALESCE(l.account_id, l.id))
    ...
    CASE
      ...
      WHEN (l.status::text = 'Reunião agendada' OR l.status::text = 'Oportunidade criada') AND o.id IS NOT NULL THEN closer_p.name
      WHEN o.id IS NOT NULL AND o.stage = 'Ganho' THEN closer_p.name
      ELSE p.name
    END AS owner_name
  FROM leads l
  LEFT JOIN LATERAL (
