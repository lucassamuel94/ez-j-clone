...
-- 1. Add opportunity_type column
ALTER TABLE opportunities ADD COLUMN IF NOT EXISTS opportunity_type text NOT NULL DEFAULT 'new_business';

-- 2. Create index for performance
CREATE INDEX IF NOT EXISTS idx_opportunities_type ON opportunities(opportunity_type);

-- 3. Mark existing evolution opportunities
UPDATE opportunities SET opportunity_type = 'evolution'
WHERE id IN (
  SELECT DISTINCT p.opportunity_id FROM proposals p
  WHERE p.product_type = 'evolucao_ez_chat' AND p.opportunity_id IS NOT NULL
);

-- 4. Update search_opportunities_paginated RPC
...
-- 5. Update get_opportunity_tab_counts RPC
...
BEGIN
  SELECT json_build_object(
    'oportunidades', (
      ...
      WHERE returned_to_sdr = false AND opportunity_type = p_opportunity_type
        AND stage NOT IN ('Ganho', 'Perdido')
        AND CASE WHEN p_closer_id = v_unassigned_sentinel THEN assigned_to_user_id IS NULL WHEN p_closer_id IS NOT NULL THEN assigned_to_user_id = p_closer_id ELSE true END
    ),
    'reunioes', (
      SELECT count(*) FROM opportunities
      WHERE returned_to_sdr = false AND opportunity_type = p_opportunity_type
...
-- 6. Update get_filtered_opportunity_ids RPC
...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE o.returned_to_sdr = false
      AND o.opportunity_type = p_opportunity_type
      AND CASE WHEN p_closer_id IS NULL THEN true WHEN p_closer_id = '00000000-0000-0000-0000-000000000000'::uuid THEN o.assigned_to_user_id IS NULL ELSE o.assigned_to_user_id = p_closer_id END
      AND CASE WHEN p_stages IS NOT NULL THEN o.stage = ANY(p_stages) WHEN p_tab = 'oportunidades' THEN o.stage NOT IN ('Ganho', 'Perdido') WHEN p_tab = 'reunioes' THEN o.stage = 'Demonstração' WHEN p_tab = 'vendas' THEN o.stage = 'Ganho' ELSE true END
      AND CASE WHEN p_meeting_from IS NOT NULL THEN o.meeting_datetime >= p_meeting_from ELSE true END
      AND CASE WHEN p_meeting_to IS NOT NULL THEN o.meeting_datetime <= p_meeting_to ELSE true END
      AND CASE WHEN p_won_from IS NOT NULL THEN o.updated_at >= p_won_from ELSE true END
      AND CASE WHEN p_won_to IS NOT NULL THEN o.updated_at <= p_won_to ELSE true END
      AND CASE
        WHEN p_search != '' THEN (
