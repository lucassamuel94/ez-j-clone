  ...
  WITH matched AS (
    SELECT
      ...
      ROW_NUMBER() OVER (
        PARTITION BY
          CASE
            WHEN l.cnpj IS NOT NULL AND l.cnpj != '' THEN
              regexp_replace(l.cnpj, '[.\-\/]', '', 'g')
            ELSE
              regexp_replace(
                lower(unaccent(COALESCE(l.razao_social, l.company))),
                '\s*(ltda|s\.?a\.?|me|epp|eireli|s/s|s\/s|ltda\.?)\s*$',
                '',
                'gi'
              )
          END
        ORDER BY
          CASE
            ...
            ELSE 3
          END,
          CASE WHEN l.cnpj IS NOT NULL AND l.cnpj != '' THEN 0 ELSE 1 END,
          l.updated_at DESC
      ) AS rn
    FROM leads l
    LEFT JOIN LATERAL (
