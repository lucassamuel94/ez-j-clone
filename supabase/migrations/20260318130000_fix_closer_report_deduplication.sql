...
-- Fix duplicate lead counting in closer productivity reports.
--
-- Problem 1 (reagendamento): createOpportunityFromMeeting was always INSERTing a new
-- opportunity, so rescheduling a meeting created a second opportunity for the same lead.
-- The service layer fix (UPDATE instead of INSERT) resolves new cases going forward.
--
-- Problem 2 (troca de closer): get_closer_activity_breakdown and get_closer_activity_metrics
-- used a UNION ALL of 6 sources for "opportunities_worked". Sources 1-4 used the historical
-- user_id of actions (notes, calls, emails, logs) while source 5 used the current
-- assigned_to_user_id. After a closer reassignment, the same lead appeared for BOTH the
-- old closer (via sources 1-4) and the new closer (via source 5), causing double-counting.
--
-- Fix: sources 1-4 now JOIN opportunities to verify the opportunity is still assigned to
-- the same user who performed the action, ensuring a lead is counted only once —
-- for whoever currently holds the assignment.
...
BEGIN
  ...
  -- All touched leads per closer.
  -- Sources 1-4 require the opportunity to still be assigned to the same user
  -- to avoid counting a lead for both old and new closer after reassignment.
  all_touched AS (
    -- Source 1: lead_notes (only if opp still assigned to note author)
    SELECT n.user_id AS cid, n.lead_id
    FROM lead_notes n
    JOIN opportunities opp ON opp.lead_id = n.lead_id
      AND opp.returned_to_sdr = false
      AND opp.assigned_to_user_id = n.user_id
    ...
    UNION ALL
    -- Source 2: call_analyses (only if opp still assigned to caller)
    SELECT ca.sdr_user_id, ca.lead_id
    FROM call_analyses ca
    JOIN opportunities opp ON opp.lead_id = ca.lead_id
      AND opp.returned_to_sdr = false
      AND opp.assigned_to_user_id = ca.sdr_user_id
    ...
    UNION ALL
    -- Source 3: sent_emails (only if opp still assigned to sender)
    SELECT se.user_id, se.lead_id
    FROM sent_emails se
    JOIN opportunities opp ON opp.lead_id = se.lead_id
      AND opp.returned_to_sdr = false
      AND opp.assigned_to_user_id = se.user_id
    ...
    UNION ALL
    -- Source 4: activity logs (only if opp still assigned to log author)
    SELECT al.user_id, al.lead_id
    FROM lead_activity_logs al
    JOIN opportunities opp ON opp.lead_id = al.lead_id
      AND opp.returned_to_sdr = false
      AND opp.assigned_to_user_id = al.user_id
    ...
    UNION ALL
    -- Source 5: meetings by current assigned_to (anchor — unchanged)
...
BEGIN
  ...
  -- Count distinct leads touched.
  -- Sources 1-4 require the opportunity to still be assigned to the acting user
  -- to avoid counting a lead for both old and new closer after reassignment.
  ...
  FROM (
    ...
    JOIN opportunities opp ON opp.lead_id = n.lead_id
      AND opp.returned_to_sdr = false
      AND (p_closer_id IS NULL OR opp.assigned_to_user_id = p_closer_id)
    WHERE (p_closer_id IS NULL OR n.user_id = p_closer_id)
      AND (p_range_start IS NULL OR n.created_at >= p_range_start)
      AND (p_range_end IS NULL OR n.created_at <= p_range_end)
