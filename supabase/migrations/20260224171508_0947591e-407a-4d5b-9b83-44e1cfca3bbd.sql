...
-- Function: paginated lead search with server-side filtering
CREATE OR REPLACE FUNCTION public.search_leads_paginated(
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE
      ...
      END
      -- Tab filter (only if no search and no advanced statuses)
      AND CASE
        ...
        WHEN p_tab = 'scheduled' THEN l.status = 'Interesse/Agendar Retorno'
        WHEN p_tab = 'confirmed' THEN l.status = 'Oportunidade criada'
        WHEN p_tab = 'future_opportunity' THEN l.status = 'Reciclagem'
  ...
  sorted AS (
    SELECT *,
      CASE WHEN status = 'Descartado' THEN -1000
           WHEN status = 'Oportunidade criada' THEN -500
           ELSE
             COALESCE(behavioral_score, 0) * 10
             + CASE WHEN is_hot_lead THEN 500 ELSE 0 END
             + CASE WHEN next_action_at < v_now THEN 200 + LEAST(EXTRACT(EPOCH FROM v_now - next_action_at) / 3600 * 5, 500) ELSE 0 END
             + CASE WHEN lead_type = 'INBOUND' AND EXTRACT(EPOCH FROM v_now - created_at) / 3600 > 4 THEN 300
                    WHEN lead_type = 'INBOUND' AND EXTRACT(EPOCH FROM v_now - created_at) / 3600 > 2 THEN 150
                    ELSE 0 END
             + CASE WHEN next_action_at >= v_now AND next_action_at <= v_end_of_today THEN 100 ELSE 0 END
      END AS calc_priority
    FROM base
    ORDER BY calc_priority DESC, next_action_at ASC
    LIMIT p_page_size OFFSET v_offset
  )
  SELECT json_build_object(
    'total', (SELECT total FROM counted),
    'data', COALESCE((SELECT json_agg(row_to_json(s)) FROM sorted s), '[]'::json)
  ) INTO v_result;
...
-- Function: get tab counts for lead inbox (lightweight - no data transfer)
CREATE OR REPLACE FUNCTION public.get_lead_tab_counts(
  p_sdr_id uuid DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'scheduled', (SELECT count(*) FROM filtered WHERE status = 'Interesse/Agendar Retorno'),
    'confirmed', (SELECT count(*) FROM filtered WHERE status = 'Oportunidade criada'),
    'future_opportunity', (SELECT count(*) FROM filtered WHERE status = 'Reciclagem'),
