...
-- 1. Unique partial index on active_clients.cnpj (ignoring NULLs and empty strings)
CREATE UNIQUE INDEX IF NOT EXISTS idx_active_clients_cnpj_unique 
ON active_clients (regexp_replace(cnpj, '\D', '', 'g'))
WHERE cnpj IS NOT NULL AND cnpj != '';

-- 2. Refactor search_all_leads_global to group by account_id
...
BEGIN
  ...
  RETURN QUERY
  SELECT DISTINCT ON (COALESCE(l.account_id, l.id))
    l.id AS lead_id,
    l.name AS lead_name,
