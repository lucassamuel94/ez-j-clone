...
DROP FUNCTION IF EXISTS public.search_opportunities_paginated(text, uuid, text, text[], timestamptz, timestamptz, timestamptz, timestamptz, int, int, text);
DROP FUNCTION IF EXISTS public.get_opportunity_tab_counts(uuid, text);

-- Recreate search_opportunities_paginated with opportunity_type filter
CREATE FUNCTION public.search_opportunities_paginated(
  p_tab text,
...
-- Recreate get_opportunity_tab_counts with opportunity_type filter
CREATE FUNCTION public.get_opportunity_tab_counts(
  p_closer_id uuid DEFAULT NULL,
  p_opportunity_type text DEFAULT 'new_business'
)
RETURNS json
